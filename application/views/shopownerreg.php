    ...
    <div class="col-md-8 offset-md-2 shadow-lg rounded p-5">
        <form method="post" id="form" class="form-horizontal" action="<?php echo site_url()?>main/user_add" enctype="multipart/form-data">
            <div class="form-group">
                ...
                <br/>
                <h2 class="text-center text-bold">Shop Owner Register</h2> <br>
                <div class="form-group">
                    <?php 
                    ...
                    <label class="text-muted">Province:</label>
                        <select style='height:40px; width: 100%' class="form-control" name="province" id="province">
                            <!--<option value="">--Choose Province--</span>-->
                            <?php
                              //foreach($provinceData as $key=>$prov){
                              //echo "<option value='".$prov['province_name']."'>".$prov['province_name']."</span>";
                              //}
                            ?>
                            </option>
                        </select> 
                    ...
                    <label class="text-muted">Municipality:</label>
                        <select style='height:40px; width: 100%' class="form-control" name="city" id="city">
                            <!--<option value="">--Choose Municipality--</span>-->
                            <?php
                              //foreach($municipalityData as $key=>$muni){
                              //echo "<option value='".$muni['municipality_name']."'>".$muni['municipality_name']."</span>";
                              //}
                            ?>
                            </option>
                        </select> 
                    ...
                    <!--<label class="text-muted">Barangay:</label>
                        <select style='height:40px; width: 100%' class="form-control" name="barangay" id="barangay">
                            <option value="">--Choose Barangay--</span>
                            <?php
                              foreach($barangayData as $key=>$brgy){
                              echo "<option value='".$brgy['barangay_name']."'>".$brgy['barangay_name']."</span>";
                              }
                            ?>
                            </option>
                        </select> -->
                    ...
                    <label class="text-muted"> Business Number </label>
                    <input type="text" name="business_number" id='business_number' class="form-control " placeholder="Enter business number">
                    <label class="text-muted">Upload Business Permit</label>
...
function save(){
    ...
    fd.append('shopname', $('#shop_name').val());
    fd.append('province', $('#province').val());
    fd.append('municipality', $('#city').val());
    //fd.append('barangay', $('#barangay').val());
    fd.append('business_number', $('#business_number').val());
    fd.append('role', 2);

    url = "<?php echo site_url('main/owner_shop_add')?>";
